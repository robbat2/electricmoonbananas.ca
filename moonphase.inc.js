...
let floor = num => Math.floor(num)
let pi = () => Math.PI
let sin = num => Math.sin(num)
let cos = num => Math.cos(num)
let tan = num => Math.tan(num)
let atan = num => Math.atan(num)
...
function jtime($timestamp) {
	let $julian = ($timestamp / 86400) + 2440587.5;	// (seconds / (seconds per day)) + julian date of epoch
	return $julian;
}
...
function phase($time = 0) {
	...
	let $M = fixangle($N + ELONGE - ELONGP);				// convert from perigee co-ordinates
	//   to epoch 1980.0
	let $Ec = kepler($M, ECCENT);					// solve equation of Kepler
	$Ec = sqrt((1 + ECCENT) / (1 - ECCENT)) * tan($Ec / 2);
	$Ec = 2 * todeg(atan($Ec));					// true anomaly
	let $Lambdasun = fixangle($Ec + ELONGP);				// Sun's geocentric ecliptic longitude
	// Orbital distance factor.
	// let $F = ( (1 + ECCENT * cos(torad($Ec))) / (1 - ECCENT * ECCENT) );
	...
	// let $MN = fixangle( MLNODE - 0.0529539 * $Day );

	// Evection.
	let $Ev = 1.2739 * sin(torad(2 * ($ml - $Lambdasun) - $MM));
	...
	let $A4 = 0.214 * sin(torad(2 * $MmP));

	// Corrected longitude.
	let $lP = $ml + $Ev + $mEc - $Ae + $A4;

	// Variation.
	let $V = 0.6583 * sin(torad(2 * ($lP - $Lambdasun)));
	...
	// Phase of the Moon.
	let $MoonPhase = (1 - cos(torad($MoonAge))) / 2;
	...
	let $pphase = $MoonPhase;
	let $mage = SYNMONTH * (fixangle($MoonAge) / 360.0);
	// 	$dist = $MoonDist;
	// 	$angdia = $MoonAng;
	// 	$sudist = $SunDist;
	// 	$suangdia = $SunAng;
